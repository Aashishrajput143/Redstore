...
from Redstore.settings import RAZORPAY_API_KEY,RAZORPAY_API_SECRET_KEY
    ...
    if(request.method=="POST"):
        ...
        if(password==cpassword):
            try:
                ...
                user.save()
                u.save()
                subject = 'Congratulations! Your Account Has been Created Successfully : Team Redstore'
                message =  """
...
def addproduct(request):
    ...
    if(request.method=="POST"):
        ...
        try:
            ...
            return HttpResponseRedirect("/profile/")
        p.save()
        subject = 'CheckOut Our Latest Products on Redstore : Team Redstore'
        message =  """
                        Hey!
...
def contact(request):
    try:
        if(request.method=='POST'):
            ...
            c.save()
            subject = 'Your Query Has been Submitted : Team Readstore'
            message =  """
...
def confirmation(request):
    buyer = Buyer.objects.get(username= request.user)
    subject = 'Your Order Has been Placed Successfully : Team Readstore'
    message =  """
                    %s
    ...
    if(request.method=='POST'):
        try:
            ...
            if(user is not None):
                ...
                subject = 'OTP for Password Reset : Team Readstore'
                message =  """
                                OTP : %d
                                Team : Redstore
                                keep shopping with us
